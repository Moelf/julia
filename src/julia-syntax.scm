                ...
                (let ((lam (scope:lam sc)))
                  (if (and lam (symbol? v))
                      (let* ((vi    (var-info-for v (car (lam:vinfo lam))))
                        ...
                        (cond (vi
                               (if (not (eq? lam inner))
                                   (begin (vinfo:set-capt! vi #t)
                                          (let loop ((s scope))
                                            (let ((s-lam (scope:lam s)))
                                              (if (not (eq? s-lam lam))
                                                  (begin
                                                    (if (pair? (scope:args s))
                                                        (set-car! (cdr (lam:vinfo s-lam))
                                                                  (cons vi (cadr (lam:vinfo s-lam)))))
                                                    (loop (scope:prev s))))))))
                               vi)
                              ((and (not (eq? lam inner))
                                    (eq? (var-kind e sc) 'static-parameter))
                               ;; add to captured vars
                               (set-car! (cdr vinfo) (cons (make-var-info e) (cadr vinfo)))
                               ;; add to static params list
                               (set-car! (cdddr vinfo) (cons e (cadddr vinfo)))
                               v)
                              (else v)))
                      global-token))
        ...
        ((eq? (car e) 'lambda)
         (let* ((args (lam:vars e))
                (newlam `(lambda ,(cadr e)
                           (,(reverse (map make-var-info args))
                            () 0 ,sp)
                           ,(lam:body e)))
                (vinfo (caddr newlam))
